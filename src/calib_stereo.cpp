...
void stereo_calibrate(std::string pathToImages, std::string filename, CalibParams& params){
    ...
    FileStorage leftFile(left_fname, FileStorage::READ);
    FileStorage rightFile(right_fname, FileStorage::READ);
...
void stereo_rectify(std::string pathToImages, std::string rectFolder, std::string filename, CalibParams& params){
    ...
    FileStorage paramsFile(filename.substr(0,dot)+"_stereo.yml", FileStorage::WRITE);
    intFile["K0"] >> K0;
	intFile["D0"] >> D0;
	intFile["K1"] >> K1;
	intFile["D1"] >> D1;
