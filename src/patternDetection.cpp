...
void drawPyramidPattern(cv::Mat& img, std::vector<cv::Point2f>& centers, bool found){
    ...
    cv::line(img, meanPT, meanPT+cv::Point2f(0,10), cv::Scalar(0,255,0), 1, cv::LINE_AA);
    cv::line(img, meanPT, meanPT+cv::Point2f(10,0), cv::Scalar(0,255,0), 1, cv::LINE_AA);
    cv::line(img, meanPT, meanPT+p1, cv::Scalar(255,255,255), 1, cv::LINE_AA);
    if(ccenters.size() > 0){
...
std::vector<std::vector<cv::Point2f>> findPattern(const std::string& pathToImages, std::vector<int>& image_idx, CalibParams& params){
    ...
    while(!img.empty()){
        ...
        switch(params.calib_pattern){
            case Pattern::CHESSBOARD:
                found = cv::findChessboardCorners(rimg, params.board_sz, corners, cv::CALIB_CB_ADAPTIVE_THRESH | cv::CALIB_CB_FILTER_QUADS);break;
            case Pattern::ASYMMETRIC_CIRCLES_GRID:
...
void findPatternStereo(const std::string& pathToImages, std::vector<std::vector<cv::Point2f>>& leftPoints, std::vector<std::vector<cv::Point2f>>& rightPoints, CalibParams& params){
    ...
    while(!limg.empty() && !rimg.empty()){
        ...
        switch(params.calib_pattern){
            case Pattern::CHESSBOARD:
                lfound = cv::findChessboardCorners(limg, params.board_sz, lcorners, cv::CALIB_CB_ADAPTIVE_THRESH | cv::CALIB_CB_FILTER_QUADS);
                rfound = cv::findChessboardCorners(rimg, params.board_sz, rcorners, cv::CALIB_CB_ADAPTIVE_THRESH | cv::CALIB_CB_FILTER_QUADS);break;
            case Pattern::ASYMMETRIC_CIRCLES_GRID:
